...
* [`src/cljs/cards_client_clj/`](src/cljs/cards_client_clj/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/cards_client_clj/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/cards_client_clj/`](test/cljs/cards_client_clj/): test files (ClojureScript,
...
7. Clone this repo and open a terminal in the `cards-client-clj` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cards_client_clj/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cards-client-clj.example
  (:require [cards-client-clj.config :as config])
